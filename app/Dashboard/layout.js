...
export default function RootLayout({ children }) {
  ...
  return (
   <>
 <div className="container w-screen flex flex-col md:flex-row min-h-screen text-black">
        {/* Top Bar (Mobile View) */}
        ...
        {/* Sidebar */}
        <div
          className={` md:w-[20%] md:h-screen flex flex-col fixed md:static top-0 left-0 h-full z-50 bg-slate-400 transition-transform duration-300 ${
            isOpen ? "translate-x-0 w-[75%] sm:w-[60%]" : "-translate-x-full"
          } md:translate-x-0 md:flex`}
        >
          {/* Sidebar Header */}
          <div className="side-bar-header flex items-center p-4 space-x-2 md:space-x-0 md:flex-col">
            <img src={Userlogo} alt="user" className="w-[45px] h-[45px] rounded-full" />
          ...
          <div className="side-bar-links flex flex-col flex-grow p-3">
            ...
            <ul className="mt-auto w-full">
              <li className="w-full">
                <button  onClick={() => signOut()} className="block bg-red-800 text-white w-full p-2 text-center rounded-md">
                  Log Out
                </button>
              </li>
            </ul>
